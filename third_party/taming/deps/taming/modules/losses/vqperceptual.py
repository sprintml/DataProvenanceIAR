...
from deps.taming.modules.discriminator.model import NLayerDiscriminator, weights_init
...
class VQLPIPSWithDiscriminator(nn.Module):
    ...
    def forward(self, codebook_loss, inputs, reconstructions, optimizer_idx,
        ...
        if optimizer_idx == 0:
            # generator update
            if self.discriminator_weight == -3:
            ...
            log = {"vqgan_loss": loss.clone().detach().mean().item(),
                   "vqgan_rec_loss": rec_loss.detach().mean().item(),
                   "vqgan_rec_loss_perceptual_component": p_loss.detach().mean().item(),
                   "vqgan_rec_loss_perceptual_weight": self.perceptual_weight,
                   "vqgan_gan_loss": g_loss.detach().mean().item(),
                   "vqgan_gan_weight": d_weight.detach().item(),
                   "vqgan_gan_factor": torch.tensor(disc_factor).item(),
                   "vqgan_codebook_loss": codebook_loss.detach().mean().item(),
                   "vqgan_codebook_weight": self.codebook_weight,
                   }
            return loss, log
