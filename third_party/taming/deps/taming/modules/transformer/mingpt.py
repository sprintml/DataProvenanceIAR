...
class GPT(nn.Module):
    ...
    def forward(self, idx, embeddings=None, targets=None):
        assert False, "In inference mode just use forward_with_past"
...
def sample_with_past(x, model, steps, temperature=1., sample_logits=True,
                     top_k=None, top_p=None, callback=None, logit_processor=None):
    # x is conditioning
    sample = x
    cond_len = x.shape[1]
    past = None
    
    # Initialize warpers once
    top_k_warper = TopKLogitsWarper(top_k=top_k) if top_k is not None else None
    top_p_warper = TopPLogitsWarper(top_p=top_p) if top_p is not None else None
    ...
    for n in range(steps):
        ...
        if past is None:
            ...
            past.append(present)
        assert logits.shape[1] == 1, "I assumed this is just 1"
        logits = logits[:, -1, :]
        if logit_processor is not None:
            logits = logit_processor(past_ids=sample, logits=logits)
        logits = logits / temperature
        
        # Apply warpers with both required parameters
        if top_k_warper is not None:
            logits = top_k_warper(input_ids=sample, scores=logits)
        if top_p_warper is not None:
            logits = top_p_warper(input_ids=sample, scores=logits)
