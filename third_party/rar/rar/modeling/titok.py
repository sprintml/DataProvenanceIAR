...
from rar.modeling.modules.base_model import BaseModel
from rar.modeling.modules.blocks import TiTokEncoder, TiTokDecoder
from rar.modeling.quantizer.quantizer import VectorQuantizer, DiagonalGaussianDistribution
from rar.modeling.modules.maskgit_vqgan import Encoder as Pixel_Eecoder
from rar.modeling.modules.maskgit_vqgan import Decoder as Pixel_Decoder
from rar.modeling.modules.maskgit_vqgan import VectorQuantizer as Pixel_Quantizer
import json
...
class PretrainedTokenizer(nn.Module):
    ...
    @torch.no_grad()
    def encode_with_internals(self, x):
        hidden_states = self.encoder(x)
        quantized_states, codebook_indices, codebook_loss = self.quantize(hidden_states, return_loss=True)
        return codebook_indices.detach(), hidden_states, quantized_states, codebook_loss

    @torch.no_grad()
    def encode_without_quant(self, x):
        hidden_states = self.encoder(x)
        quantized_states, codebook_indices, codebook_loss = self.quantize(hidden_states.clone(), return_loss=True)
        return codebook_indices.detach(), hidden_states.clone(), hidden_states.clone(), codebook_loss
    ...
    def decode_states(self, states):
        rec_images = self.decoder(states)
        rec_images = torch.clamp(rec_images, 0.0, 1.0)
        return rec_images.detach()
